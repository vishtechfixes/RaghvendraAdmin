...
const SHOP_ID   = 'raghvendra-sweets';   // shared/constants.js se match karo
...
function fillInputs(sh, s) {
  ...
  sv('o-reward',s.defaultVisitReward    || 'FREE Mithai 250g');
...
window.saveAll = async function () {
  ...
  try {
    ...
    const settingsData = {
      ...
      defaultVisitReward:    gv('o-reward').trim()    || 'FREE Mithai 250g',
...
window.delBills = function () {
  ...
  localStorage.removeItem('rs_bills');
...
window.delAll = function () {
  ...
  ['rs_users','rs_bills','rs_menu','rs_settings',
   'rs_shop','rs_feedback','rs_pts_history'].forEach(k =>
